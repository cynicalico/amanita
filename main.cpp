...
Fungespace::FixedCoord_ Fungespace::make_fixed_coord_(std::int64_t x, std::int64_t y) const {
    if (x < 0) {
        if (y < 0) {
            return FixedCoord_{
                .x = static_cast<std::size_t>(-x - 1), .y = static_cast<std::size_t>(-y - 1),
                .quadrant = const_cast<std::vector<Cell> &>(nx_ny_)
            };
        }
        return FixedCoord_{
            .x = static_cast<std::size_t>(-x - 1), .y = static_cast<std::size_t>(y),
            .quadrant = const_cast<std::vector<Cell> &>(nx_py_)
        };
    }
    if (y < 0) {
        return FixedCoord_{
            .x = static_cast<std::size_t>(x), .y = static_cast<std::size_t>(-y - 1),
            .quadrant = const_cast<std::vector<Cell> &>(px_ny_)
        };
    }
    return FixedCoord_{
        .x = static_cast<std::size_t>(x), .y = static_cast<std::size_t>(y),
