...
Interpreter::Interpreter(const std::filesystem::path &path): fungespace(path) {
}
...
InstructionAction Interpreter::perform_instruction(Instruction ins, Fungespace &fungespace, InstructionPointer &ip) {
    if (ip.stringmode) {
    ...
    switch (ins) {
        ...
        default: {
            fmt::println("Unknown instruction: {}/{}, reflecting", static_cast<std::int64_t>(ins),
...
void Interpreter::step_wrap(const Fungespace &fungespace, InstructionPointer &ip) {
    ip.step();
...
void Interpreter::step_to_next_instruction(const Fungespace &fungespace, InstructionPointer &ip, Cell prev_ins,
                                           bool start_skipping) {
